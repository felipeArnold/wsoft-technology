<x-layouts.pdf :title="'Contas a Pagar'">
            ...
            <tr>
                <td style="width: 25%; padding: 10px;">
                    <div style="background-color: #f3f4f6; border-left: 4px solid #6b7280; padding: 15px; border-radius: 5px;">
                        <div style="font-size: 10px; color: #4b5563; font-weight: bold; margin-bottom: 5px;">TOTAL</div>
                        <div style="font-size: 16px; color: #1f2937; font-weight: bold;">R$ {{ number_format($totalAmount, 2, ',', '.') }}</div>
                    </div>
                </td>
                <td style="width: 25%; padding: 10px;">
                    <div style="background-color: #d1fae5; border-left: 4px solid #059669; padding: 15px; border-radius: 5px;">
                        <div style="font-size: 10px; color: #047857; font-weight: bold; margin-bottom: 5px;">PAGO</div>
                ...
                <td style="width: 25%; padding: 10px;">
                    <div style="background-color: #fef3c7; border-left: 4px solid #b45309; padding: 15px; border-radius: 5px;">
                        <div style="font-size: 10px; color: #92400e; font-weight: bold; margin-bottom: 5px;">PENDENTE</div>
    ...
    <div class="section">
        ...
        <table style="width: 100%; border-collapse: collapse; font-size: 9px; margin-top: 10px;">
            <thead>
                <tr style="background-color: #374151; color: white;">
                    <th style="padding: 8px; text-align: left; border: 1px solid #4b5563;">Fornecedor</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom: 1px solid #e5e7eb;">
                    ...
                    <td style="padding: 6px; text-align: right; border: 1px solid #e5e7eb; font-weight: bold;">R$ {{ number_format($account->total_amount, 2, ',', '.') }}</td>
                    ...
                    <td style="padding: 6px; text-align: center; border: 1px solid #e5e7eb;">
                        ...
                        @elseif($account->installments->where('status', 1)->count() === $account->parcels)
                            <span style="background-color: #d1fae5; color: #059669; padding: 3px 8px; border-radius: 10px; font-size: 8px; font-weight: bold;">PAGO</span>
                        @else
        ...
        @else
        <div class="alert-info">Nenhuma conta a pagar encontrada no período selecionado.</div>
        @endif
    </div>

    <x-pdf.footer
        :documentNumber="'#CP-' . now()->format('YmdHis')"
        documentType="Contas a Pagar"
    />
