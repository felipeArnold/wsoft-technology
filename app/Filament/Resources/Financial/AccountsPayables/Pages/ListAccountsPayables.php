...
final class ListAccountsPayables extends ListRecords
{
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Action::make('export_payables_pdf')
                ->label('Exportar PDF')
                ->icon('heroicon-o-document-arrow-down')
                ->color(Color::Gray)
                ->form([
                    Section::make('Período de Exportação')
                        ->description('Selecione o período para exportar as contas a pagar.')
                ...
                ->action(function (array $data): Response {
                    ...
                    $pdf = Pdf::loadView('pdf.accounts-payables', [
                        'tenant' => $tenant,
                        'startDate' => $startDate,
                        'endDate' => $endDate,
                        'accounts' => $accounts,
                    ...
                    return response()->streamDownload(
                        fn () => print ($pdf->output()),
                        'contas-pagar-'.now()->format('Y-m-d').'.pdf'
                    );
                }),
        ];
    }
